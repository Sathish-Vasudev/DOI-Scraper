...
while True:
    ...
    #GETTING FILE NAME AND CREATING EXCEL FILE
    Excelname = input("Enter the excel file name (*.xlsx): \n")
    if Excelname == "" :
        Excelname = "downloadedfiles.xlsx"
    elif Excelname.find("xlsx") == -1:
        Excelname =  Excelname + ".xlsx"
    else :
        Excelname = Excelname

    Excelfilepath = filepath + foldrname + Excelname

    dat = {'DOI':[''],'TITLE':[''],'AUTHOR':[''],
            'PUBLISHER':[''],'YEAR':[''],'URL':[''],'FILE NAME':[''],
            'FILE PATH':[''],'CITATION':['']}
    datf = pd.DataFrame(dat,columns = ['DOI','TITLE','AUTHOR','PUBLISHER','YEAR','URL','FILE NAME','FILE PATH','CITATION'])
    excelw = pd.ExcelWriter(Excelfilepath, engine='xlsxwriter')
    ...
    for doi in selecteddois:
        ...
        try:
            ...
            artlnk = artcl['src']
            artlnki = artlnk.rindex('pdf')
            artlnk = artlnk[0:artlnki+3:]
            ...
            if len(tit)>10:
                ...
                auth = "No Data Available"
                title = "No Data Available"
                pub = "No Data Available"
                yr = "No Data Available"
                tit = "No Data Available"
        ...
        try:
            with open(filepath + foldrname + artname, "wb") as file:
                ...
                dat = {'DOI':[doi],'TITLE':[title],'AUTHOR':[auth],
                       'PUBLISHER':[pub],'YEAR':[yr],'URL':[artlnk],'FILE NAME':[artname],
                        'FILE PATH':[filepath + foldrname + artname],'CITATION':[titl]}
                datf = pd.DataFrame(dat,columns = ['DOI','TITLE','AUTHOR','PUBLISHER','YEAR','URL','FILE NAME','FILE PATH','CITATION'])
                writer = pd.ExcelWriter(Excelfilepath, engine='openpyxl',mode='a')
                writer.book = load_workbook(Excelfilepath)
                ws = writer.book.active
                writer.sheets = dict((ws.title, ws) for ws in writer.book.worksheets)
                reader = pd.read_excel(Excelfilepath)
                datf.to_excel(writer,index=False,header=False,startrow=len(reader)+1)
                writer.save()
